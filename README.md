...
| Package | Use it for |
| --- | --- |
| `oxhq/cachelet` | Full suite: core + model + query + request + cloud integrations |
...
| `oxhq/cachelet-cloud` | First-party exporter for canonical Cachelet telemetry |
...
`oxhq/cachelet-cloud` listens to that telemetry stream and exports `cachelet.cloud.export.v1` payloads through `http`, `log`, `null`, or custom transports.
