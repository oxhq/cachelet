...
it('can cache null payloads without treating them as misses', function () {
    $builder = Cachelet::for('nullable')->from(['id' => 12])->ttl(60);

    $runs = 0;

    $first = $builder->remember(function () use (&$runs) {
        $runs++;

        return null;
    });

    $second = $builder->fetch(function () use (&$runs) {
        $runs++;
